...
class ComparisonTableGenerator:
    ...
    def _load_complete_metadata(self, summary_file: Path) -> Dict:
        """Load fields missing from legacy summary JSON files."""
        complete_name = summary_file.name.replace('_summary.json', '_complete.json')
        complete_file = summary_file.parent / 'json' / complete_name
    ...
    def collect_all_results(self) -> pd.DataFrame:
        ...
        for json_file in self.results_dir.rglob('*_summary.json'):
            ...
            try:
                ...
                complete_metadata = self._load_complete_metadata(json_file)
                ...
                result = {
                    ...
                    'MAE_mean': mae_mean,
                    'MAE_std': mae_std,
                    'n_samples': data.get('n_samples') or complete_metadata.get('n_samples', 0),
